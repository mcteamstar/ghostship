...
`GA_PORTSIDE_ENABLED=true` is an opt-in, breaking cutover — no coexistence with the existing per-port mode. Re-run `install.sh` after enabling.

- `GA_PORTSIDE_ENABLED=true` adds a `ga-portside` container (vanilla `caddy:2` image) to the compose stack. It becomes the sole TLS terminator and auth gate for all ports.
...
- **vm23 host Caddy retirement:** `ga-portside` replaces the host-level Caddy on vm23. Stop and disable the host Caddy before or alongside the `install.sh` cutover.
- New env vars: `GA_PORTSIDE_ENABLED`, `GA_PORTSIDE_ADMIN_URL`, `GA_PORTSIDE_TLS_MODE`, `GA_PORTSIDE_DOMAIN`, `GA_PORTSIDE_PORT`, `GA_PORTSIDE_HTTP_PORT`, `GA_PORTSIDE_SESSION_TTL_SECS`.
