...
| <img src="docs/images/tool-dispatch.png" width="256"> | `dispatch` | Spawn a task on one of the six agent personas via the KiroCrew spawn API — lightweight, immediate, no persistent session attached. Returns a `task_id`. Pass `tasks=[...]` for atomic batch dispatch; response includes `batch_id` and per-task `task_ids`. By default tasks run headless (`slot=None`); pass `slot="bridge"` or `slot=True` to attach to a dashboard session for browser visibility (costs ~300–400 MB per slot process). |
