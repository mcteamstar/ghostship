...
2. **Task-description signal (secondary):** Cross-check the `task` field in `kirocrew spawn list` for an in-flight task whose description begins with the stable marker `SDD dispatch <change> <persona> <intent_id>`. Match the change, target persona, and intent token; do not use the `agent` field as this check.
