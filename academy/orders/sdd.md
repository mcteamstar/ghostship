...
2. **Task-description signal (secondary):** Cross-check the `task` field in `kirocrew spawn list` for an in-flight task whose description contains `<change>` and `<persona>`. Match on whatever portion of the description is visible; `kirocrew spawn list` caps this field at 80 characters, and `<change>`/`<persona>` can still be truncated away for a long change ID — this signal is best-effort corroboration only, not authoritative, and never the sole basis to hold or proceed. Do not use the `agent` field for this check.
...
After all three signals are clear and the SDD transition requires a dispatch, apply the intent UUID idempotency check before generating a new `intent_id`:

**Intent UUID idempotency check:** Check whether the raven mailbox already contains a pending or confirmed intent for the target persona (a `dispatching <persona> <intent_id>` subject line in `/var/mail/raven/new/` or `/var/mail/raven/cur/`). If such a record exists and is not yet stale (per the staleness rules above), that `intent_id` is already the canonical idempotency key for this dispatch. Scan `kirocrew spawn list` task descriptions for that specific `intent_id` substring — it always appears intact immediately after the `SDD dispatch` prefix, never truncated away; if found, the spawn already happened — steer or continue that task rather than generating a new `intent_id` and spawning again. Only generate a fresh `intent_id` when no non-stale intent record exists for this dispatch.

Once the idempotency check confirms no prior intent is active, generate a unique local `intent_id` in the form `intent-<uuid>` and put it at the start of the worker task description as `SDD dispatch <intent_id> <change> <persona>` — `intent_id` must come immediately after the literal `SDD dispatch` prefix, before `<change>` and `<persona>`, so its fixed length (`intent-` plus a 36-character UUID) always survives `kirocrew spawn list`'s 80-character field cap regardless of how long the change ID is. Before calling `/api/spawn`, write a pending dispatch-intent message to `raven@localhost` with `To: raven@localhost`, `From: raven+<raven_task_id>@localhost`, and subject `dispatching <persona> <intent_id>`. The gateway assigns the real spawn task ID inside `/api/spawn`, so do not invent or claim that ID before the call.
