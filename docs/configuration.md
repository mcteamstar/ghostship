...
| `GA_CADDY_ENABLED` | `false` | Opt into the Caddy reverse-proxy layer (TRN-92). When `true`, a `ga-caddy` container is added to the compose stack; it binds ports 443/80 **and** the dashboard port range and becomes the sole TLS terminator and auth gate. The transport's per-port uvicorn listeners are not started. **Breaking** — re-run `install.sh` after changing this value; no coexistence with the old per-port mode. See [caddy.md](caddy.md) |
| `GA_CADDY_ADMIN_URL` | `http://ga-caddy:2019` | URL of the Caddy admin API reachable from inside the transport container. Override when running Caddy at a non-default address |
