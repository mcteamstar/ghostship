...
| `GA_CREW_ACP_BACKEND` | `kiro` | ACP runtime used inside crew containers. Valid values: `kiro` (default, existing behaviour), `claude` (Claude Code ACP backend). When `claude`, kiro-cli auth injection is skipped and `ANTHROPIC_API_KEY` is injected instead. Requires `GA_CREW_ANTHROPIC_API_KEY` to be set and a crew image built with `GA_INCLUDE_CLAUDE_AGENT=true`. Unrecognised values abort startup with a `ConfigError`. |
| `GA_CREW_ANTHROPIC_API_KEY` | _(unset)_ | Anthropic API key injected as `ANTHROPIC_API_KEY` into crew containers when `GA_CREW_ACP_BACKEND=claude`. **Required** when `GA_CREW_ACP_BACKEND=claude` — transport exits with `ConfigError` at startup if unset. **Never log, print, or embed this value.** |
