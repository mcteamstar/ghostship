...
| `GA_PUBLIC_URL` | `http://localhost:<PORT>` | Base URL baked into presigned `evac`/`supply` links. Replaces the deprecated `GA_MCP_PUBLIC_URL` and `GA_FILE_PUBLIC_URL` variables — set this single var for all externally-reachable URLs |
| `GA_MCP_PUBLIC_URL` | _(unset)_ | **DEPRECATED.** Legacy fallback for `GA_PUBLIC_URL`. If `GA_PUBLIC_URL` is unset and this is set, it is used with a deprecation warning. Migrate to `GA_PUBLIC_URL` |
| `GA_FILE_PUBLIC_URL` | _(unset)_ | **DEPRECATED.** No longer read by the transport. Retained in `install.sh --file-public-url` for backward-compatible config files only. Migrate to `GA_PUBLIC_URL` |
...
| `GA_API_KEY` | `--api-key` |
| `GA_PUBLIC_URL` | `--public-url` |
| `GA_FILE_PUBLIC_URL` | `--file-public-url` _(deprecated, migrate to `GA_PUBLIC_URL`)_ |
| `GA_MCP_PUBLIC_URL` | `--mcp-public-url` _(deprecated, migrate to `GA_PUBLIC_URL`)_ |
...
GA_PUBLIC_URL="https://academy.example.com"
```
...
> with a single `GA_PUBLIC_URL` pointing at the unified endpoint.
