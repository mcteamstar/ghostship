...
| `GA_LIGHTHOUSE_ENABLED` | `false` | Master switch for **ga-lighthouse**, the optional read-only fleet dashboard container. When `false` (default), no `ga-lighthouse` service is emitted into `compose.yml`, no `/lighthouse/*` Caddy route is generated, and the transport does **not** register the `GET /api/crews` or `GET /api/crews/{crew_id}/mail` REST endpoints (they return `404`). Set to `true` (or `1`/`yes`/`on`) to build the lighthouse image (done automatically by `install.sh`) and serve the fleet view at `<portal-url>/lighthouse/` — crew cards, per-persona mail badges, and host memory in one browser view. Accepted values: `true` / `false`. Config-file-only. |
...
| `GA_CREW_ACP_BACKEND` | `kiro` | ACP runtime used inside crew containers. Valid values: `kiro` (default, existing behaviour), `claude` (Claude Code ACP backend), `codex` (Codex ACP backend). When `claude`, kiro-cli auth injection is skipped; credentials are supplied via `GA_CREW_ANTHROPIC_API_KEY` (API key path) or Claude OAuth login (OAuth path). When `codex`, kiro-cli auth injection is skipped; credentials are supplied via `GA_CREW_OPENAI_API_KEY` (API key path) or Codex OAuth login (`ga-codex-auth`). `claude` requires a crew image built with `GA_INCLUDE_CLAUDE_AGENT=true`; `codex` requires `GA_INCLUDE_CODEX_AGENT=true`. Unrecognised values abort startup with a `ConfigError`. |
| `GA_CREW_ANTHROPIC_API_KEY` | _(unset)_ | Anthropic API key injected as `ANTHROPIC_API_KEY` into crew containers when `GA_CREW_ACP_BACKEND=claude`. **Optional from TRN-170** — when unset, Claude OAuth credentials (`ga-claude-auth`) are used instead. When both are present, the API key takes precedence. If neither is present at `launch` time, `launch()` returns `not_authenticated` with a login URL. **Never log, print, or embed this value.** |
| `GA_CREW_ANTHROPIC_BASE_URL` | _(unset)_ | Optional Anthropic-compatible endpoint URL injected as `ANTHROPIC_BASE_URL` into crew containers when `GA_CREW_ACP_BACKEND=claude`. When set, allows operators to redirect Claude-backend crew traffic to a local LLM proxy (litellm, OpenRouter, LM Studio, etc.) without image changes. Has no effect when `GA_CREW_ACP_BACKEND != "claude"`. |
| `GA_INCLUDE_CLAUDE_AGENT` | `false` | When `true`, `install.sh` passes `--build-arg INCLUDE_CLAUDE_AGENT=true` to the spec-ops image build, which installs `@anthropic-ai/claude-code@0.2.93`. Must be set in addition to `GA_CREW_ACP_BACKEND=claude`. |
| `GA_CREW_OPENAI_API_KEY` | _(unset)_ | OpenAI API key injected as `OPENAI_API_KEY` into crew containers when `GA_CREW_ACP_BACKEND=codex`. **Optional** — when unset, Codex OAuth credentials (`ga-codex-auth`) are used instead. When both are present, the API key takes precedence. If neither is present at `launch` time, `launch()` returns `not_authenticated` with a login URL (mirrors the TRN-170 Claude model). **Never log, print, or embed this value.** |
| `GA_CREW_OPENAI_BASE_URL` | _(unset)_ | Optional OpenAI-compatible endpoint URL injected as `OPENAI_BASE_URL` into crew containers when `GA_CREW_ACP_BACKEND=codex`. When set, redirects Codex-backend crew traffic to an OpenAI-compatible endpoint without image changes, and the `launch()` network WARNING names this endpoint instead of `api.openai.com`. Has no effect when `GA_CREW_ACP_BACKEND != "codex"`. |
| `GA_INCLUDE_CODEX_AGENT` | `false` | When `true`, `install.sh` passes `--build-arg INCLUDE_CODEX_AGENT=true` to the spec-ops image build, which installs `@agentclientprotocol/codex-acp@1.12.0` (ONE package — the adapter ships its own Codex binary, so there is no separate `codex` CLI). Must be set in addition to `GA_CREW_ACP_BACKEND=codex`. |
