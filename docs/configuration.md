...
| `GA_PORTAL_PORT` | `443` | HTTPS port Caddy listens on for the main server (MCP, files, auth endpoints) |
| `GA_PORTAL_HTTP_PORT` | `80` | HTTP port Caddy listens on for ACME challenges and HTTP→HTTPS redirects |
...
| `GA_PORTAL_SECRET` | _(auto-generated by `install.sh`)_ | **Not user-settable directly.** Shared secret between `ga-portal` (Caddy) and `ga-transport`. `install.sh` generates it with `openssl rand -hex 32` and stores it as Podman secret `ga-portal-secret`. Caddy injects it as `X-Portal-Token` on every upstream request; transport rejects requests missing or presenting a wrong token with HTTP 401 (TRN-107). The secret is idempotent — preserved across reinstalls so the portal and transport stay in sync. To rotate it, delete the `ga-portal-secret` Podman secret and re-run `install.sh`. |
...
> at the application layer by `GA_PORTAL_SECRET`. `ga-portal` cannot reach crew containers
