...
| `GA_CREW_ACP_BACKEND` | `kiro` | ACP runtime used inside crew containers. Valid values: `kiro` (default, existing behaviour), `claude` (Claude Code ACP backend). When `claude`, kiro-cli auth injection is skipped; credentials are supplied via `GA_CREW_ANTHROPIC_API_KEY` (API key path) or Claude OAuth login (OAuth path). Requires a crew image built with `GA_INCLUDE_CLAUDE_AGENT=true`. Unrecognised values abort startup with a `ConfigError`. |
