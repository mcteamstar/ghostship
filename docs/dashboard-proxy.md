...
- **Caddy talks only to `ga-transport:64057`.** Both the MCP/file routes and the per-crew dashboard routes upstream to `ga-transport:64057` (the transport's fixed internal port). Caddy has no network path to crew containers (`gs-*`) — see [Network topology](#network-topology).
...
- **`gs_session` cookie gate.** When `GA_API_KEY` is set, every dashboard port requires a valid `gs_session` cookie issued by `/dashboard/login`. Unauthenticated requests redirect to the login page (`/dashboard/login`).
...
- **CORS is pre-configured.** The UI port origin (`{scheme}://{host}:{dashboard_port}`) is added to `KIROCREW_CORS_ORIGINS` at container create time. This is the origin the browser uses, so the SPA's API calls are accepted; the transport's internal port (`64057`) is never a browser-facing origin and needs no CORS entry.
...
- `ga-portal` (Caddy) is on `ga-portside` only. It reaches `ga-transport:64057` over `ga-portside` and has no route to any crew container. The trust boundary — the external-facing proxy cannot touch crew gateways directly — is enforced by network topology, not just config.
