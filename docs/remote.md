...
- **Port exposure** — the MCP port (default `64057`) and file-transfer port
...
| `--file-public-url <url>` | Base URL baked into presigned `evac`/`supply` links (set to the file port's public HTTPS address) |
| `--port <port>` | Override the MCP listen port (file server follows at `port+1`) |
...
- **File-transfer port: HMAC-only, no TLS natively** — the file server on
  `PORT+1` uses HMAC presigned URLs for authorization but does not speak
