...
- Replace `GA_MCP_PUBLIC_URL` + `GA_FILE_PUBLIC_URL` with a single `GA_HOST_URL`
- Update presigned URL generation (`supply`, `evac`) to use `GA_HOST_URL`
...
- Update `docs/configuration.md` — remove old vars, add `GA_HOST_URL`
- Migration note: `GA_FILE_PUBLIC_URL` removed; existing installs must set `GA_HOST_URL`

## Capabilities

### Modified Capabilities
- `mcp-server`: file transfer endpoints unified onto the main transport port; `GA_HOST_URL` replaces the split URL config
