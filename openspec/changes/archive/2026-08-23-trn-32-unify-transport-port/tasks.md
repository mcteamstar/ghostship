...
- [x] 1.4 Add `GA_HOST_URL` env var reading: prefer `GA_HOST_URL`, fall back to `GA_MCP_PUBLIC_URL` with deprecation warning, default to `http://localhost:{PORT}`
- [x] 1.5 Update presigned URL generation in `supply()` and `evac()` to use `GA_HOST_URL`
...
- [x] 3.1 Add `GA_HOST_URL` entry (replaces both split URL vars)
...
- [x] 4.2 Add test: `evac()` presigned URL uses `GA_HOST_URL` base
- [x] 4.3 Add test: fallback to `GA_MCP_PUBLIC_URL` when `GA_HOST_URL` unset emits deprecation warning
