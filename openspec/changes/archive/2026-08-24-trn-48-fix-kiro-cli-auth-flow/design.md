...
- Confirmed live (this session, against a real IAM Identity Center IDC login): with `KIRO_IDENTITY_PROVIDER`/`KIRO_REGION` set, this path works end-to-end — `POST /login` returns a valid device URL, `GET /login` reports `complete` after the browser flow, and `ga-kiro-auth` is written.
