...
Protects the dashboard login form (`GET /login-ui` → `POST /dashboard-login`) against cross-site request forgery by embedding a server-generated token in the form and validating it on submission.
...
The transport SHALL embed the CSRF token as a hidden form field named `csrf_token` in the HTML returned by `GET /login-ui`.

#### Scenario: Token present in rendered form
- **WHEN** a client requests `GET /login-ui`
