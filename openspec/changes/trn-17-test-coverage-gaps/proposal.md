...
## Also: Test suite portability inside crew containers

The full test suite (`python -m unittest transport.test_transport`) consistently
times out at exactly 30 minutes inside crew containers (on Academy/HyperV). The
cause is almost certainly one or more tests that call real `podman` commands —
those tests hang indefinitely inside a crew container where Podman is either
absent or the socket path is wrong.

TRN-17 should:
- Identify which test classes/methods require a real Podman socket vs which are
  pure unit tests (mocked)
- Add a `@unittest.skipUnless(shutil.which("podman"), "requires podman")` guard
  or equivalent to all Podman-dependent tests
- Produce a known-safe subset that can be run inside crew containers (targeted
  test runs for Ghost/Banshee during SDD cycles)
- Document the split in a comment at the top of `test_transport.py`
