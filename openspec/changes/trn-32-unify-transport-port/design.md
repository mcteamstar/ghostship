...
`GA_MCP_PUBLIC_URL` for MCP/REST. These need to collapse to `GA_PUBLIC_URL`.

## Goals / Non-Goals

**Goals:**
- Single port, single Starlette app, single public URL var
- Backward compat: `GA_MCP_PUBLIC_URL` as fallback if `GA_PUBLIC_URL` unset
...
### 2. GA_PUBLIC_URL with GA_MCP_PUBLIC_URL fallback

**Choice:** Read `GA_PUBLIC_URL` first; if absent, fall back to
...
2. `ghostship.conf` migration: rename `GA_MCP_PUBLIC_URL` → `GA_PUBLIC_URL`
