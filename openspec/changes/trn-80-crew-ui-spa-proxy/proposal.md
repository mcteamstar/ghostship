...
- **Opt-in at launch** — `launch(dashboard=True)` allocates a dedicated port from a configurable range (`GA_UI_PORT_RANGE_START`, default 64058). Defaults to `False` — agent crews are headless by default, no ports opened.
...
- `launch` response and `crews` list include `ui_url` when a dashboard is active (`null` otherwise).
...
- `transport/config.py` — `GA_UI_PORT_RANGE_START` (default 64058), `GA_UI_PORT_RANGE_SIZE` (default 50), `GA_UI_PORT_ENABLED` (global on/off).
