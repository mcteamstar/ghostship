...
- **WHEN** a config file exports any combination of: `KIRO_IDENTITY_PROVIDER`, `KIRO_REGION`, `KIRO_LICENSE`, `PORT`, `KC_MODEL_OVERRIDE`, `GA_API_KEY`, `GA_FILE_PUBLIC_URL`, `GA_MCP_PUBLIC_URL`
...
`docs/configuration.md` SHALL document the config file format (shell file exporting variables), list all supported variables, and state the resolution order (config file → flags → defaults).

#### Scenario: Config file section present in docs
- **WHEN** reading `docs/configuration.md`
- **THEN** it SHALL contain a "Config file" section explaining that the file is a shell script exporting variables, listing all supported variable names, and stating the resolution order: config file → command-line flags → defaults
