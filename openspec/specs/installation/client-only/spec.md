# installation/client-only Specification
...
- **WHEN** `scripts/install.sh --client-only --url https://academy.penguin-piano.ts.net/mcp` is invoked
- **THEN** `ghostship setup` is called with `--url https://academy.penguin-piano.ts.net/mcp`
