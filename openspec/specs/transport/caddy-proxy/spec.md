...
When `GA_PORTSIDE_ENABLED=true`, `install.sh` SHALL add a `ga-portside` service to the generated `compose.yml`. The service SHALL bind the public HTTPS port (default 443), the HTTP port (default 80) for ACME challenges and redirects, AND the dashboard port range (default 64058–64107) — Caddy owns those port bindings, not the transport. The Caddy admin API (port 2019) SHALL be bound to the internal `ga-net` network only and SHALL NOT be published to the host. The `ga-transport` service SHALL NOT bind the dashboard port range when `GA_PORTSIDE_ENABLED=true`.

#### Scenario: Fresh install with Caddy enabled
- **WHEN** `install.sh` runs with `GA_PORTSIDE_ENABLED=true`
- **THEN** `compose.yml` contains a `ga-portside` service with the `caddy:2` image, ports `80:80`, `443:443`, and the dashboard range, and no `2019:2019` host mapping
- **THEN** the `ga-transport` service does not bind the dashboard port range

#### Scenario: Fresh install with Caddy disabled
- **WHEN** `install.sh` runs with `GA_PORTSIDE_ENABLED=false` (the default)
- **THEN** `compose.yml` does not contain a `ga-portside` service and transport behavior is unchanged (the transport binds the dashboard range as today)
...
- **WHEN** the `ga-portside` container starts for the first time
...
When `GA_PORTSIDE_ENABLED=true` and `GA_API_KEY` is set, the main-server `/mcp*` and `/files/*` routes SHALL require an `Authorization: Bearer <GA_API_KEY>` header. A request without the correct token SHALL be rejected by Caddy with 401 and `WWW-Authenticate: Bearer`, and SHALL NOT reach the transport process. The transport's own `BearerAuthMiddleware` SHALL remain active for defence in depth.
...
- **WHEN** `launch(crew_id="alpha", dashboard=True)` completes and `GA_PORTSIDE_ENABLED=false`
...
When a crew is nuked and `GA_PORTSIDE_ENABLED=true`, the transport SHALL call `DELETE /config/id/crew-{crew_id}` on the Caddy admin API to remove the corresponding server, and release the port back to the pool. Failure to contact the Caddy admin API SHALL be logged but SHALL NOT cause `nuke` to fail.
...
On startup, when `GA_PORTSIDE_ENABLED=true`, the transport SHALL re-register a Caddy dashboard server for every crew in `crews.json` that has an allocated dashboard port. Re-registration SHALL be idempotent — an existing `@id` SHALL be handled gracefully.
