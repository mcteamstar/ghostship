...
podman machine list
# Expected: two machines — "default" (or whatever existed before) AND "ghostship"
...
**Pass criteria:** `podman machine list` shows `ghostship`, transport is
...
podman --root=~/.local/share/ghostship/containers/storage \
  --runroot=$XDG_RUNTIME_DIR/ghostship-containers ps
...
podman machine list
# Expected: "ghostship" machine is gone

# 3. Verify cleanup (Linux)
systemctl --user status podman-ghostship.socket 2>&1
# Expected: "could not be found" or "inactive"
ls ~/.local/share/ghostship/ 2>&1
...
# macOS:
podman machine list  # ghostship still present
# Linux:
ls ~/.local/share/ghostship/  # directory preserved
```
...
# Expect to see default + ghostship connections

# Query dedicated machine directly:
podman --connection ghostship ps
