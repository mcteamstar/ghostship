...
# When GA_PORTSIDE_ENABLED=true, a ga-portside container owns port bindings for
...
GA_PORTSIDE_ENABLED = cfg.ga_portside_enabled
...
def _caddy_admin_url() -> str:
    ...
    return cfg.ga_portside_admin_url
...
def _caddy_register_crew(crew_id: str, port: int) -> None:
    ...
    carrying a ``forward_auth``-equivalent check against ``/dashboard-auth`` and a
    ``reverse_proxy`` to ``gs-{crew_id}:5476``. PUT to the Caddy admin API.

    The ``forward_auth`` Caddyfile directive is not available as a JSON handler
    in standard ``caddy:2``; instead we use the equivalent JSON that ``caddy adapt``
    produces: a ``reverse_proxy`` with ``rewrite``, ``headers``, and ``handle_response``
    that together implement the forward-auth flow using only built-in modules.
    ...
    #   forward_auth http://ga-transport:8000 {
    #       uri /dashboard-auth
    #       copy_headers X-Crew-Cookie
    #   }
    # Generated by: caddy adapt --config <caddyfile>
...
def _gs_session_issue() -> str:
    ...
    expiry = time.time() + cfg.ga_portside_session_ttl_secs
...
async def _handle_dashboard_auth(request: Request) -> Response:
    ...
    # In Caddy mode the forward_auth call comes from ga-portside → ga-transport,
...
def _start_dashboard_port_server(port: int, crew_id: str, app: Any) -> None:
    ...
    TRN-92: When GA_PORTSIDE_ENABLED=True, Caddy owns the port binding —
...
async def _handle_crew_dashboard_post(request: Request) -> Response:
    ...
    if GA_PORTSIDE_ENABLED:
        ...
        # GA_PORTSIDE_TLS_MODE=off means plain HTTP; all other modes use HTTPS.
        _caddy_scheme = "http" if cfg.ga_portside_tls_mode == "off" else "https"
        if cfg.ga_host_url:
    ...
    logger.info(
        ...
        crew_id, dashboard_port, dashboard_url, GA_PORTSIDE_ENABLED,
    )
    return JSONResponse({"dashboard_url": dashboard_url})
...
def crews() -> dict:
    ...
    for cid, info in reg["crews"].items():
        ...
        # TRN-92: use HTTPS scheme when GA_PORTSIDE_ENABLED=True
